...
describe('install', () => {
  describe('dependencies', () => {
    ...
    it('with yarn without passing dependencies', async () => {
      const mockSpawn = require('mock-spawn')();
      spawn.mockImplementationOnce(mockSpawn);
      const useYarn = true;
      const isOnline = true;
      await install('test-module', null, { useYarn, isOnline });
      expect(mockSpawn.calls[0].command).toEqual('yarnpkg');
      expect(mockSpawn.calls[0].args).toMatchSnapshot();
    });

    it('with npm without passing dependencies', async () => {
      const mockSpawn = require('mock-spawn')();
      spawn.mockImplementationOnce(mockSpawn);
      const useYarn = false;
      const isOnline = true;
    ...
    it('with yarn while offline', async () => {
      const mockSpawn = require('mock-spawn')();
      spawn.mockImplementationOnce(mockSpawn);
      const useYarn = true;
    ...
    it('fails with error message if exit code is not 0', async () => {
      expect.assertions(2);
      const mockSpawn = require('mock-spawn')();
  ...
  describe('devDependencies', () => {
    ...
    it('with yarn without passing dependencies', async () => {
      ...
      const useYarn = true;
      const isOnline = true;
      await installDevDependencies('test-module', null, { useYarn, isOnline });
