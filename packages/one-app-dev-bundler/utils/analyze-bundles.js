...
const extractStats = (bundler, content) => {
  ...
  switch (bundler) {
    case 'webpack': {
      const data = content.assets[0]; // Note: we only accept one file at the moment
    ...
    case 'esbuild': {
      ...
      const data = content[Object.keys(content)[0]]; // Note: we only accept one file at the moment
...
const saveReport = async (stats) => {
  ...
  try {
    ...
    console.log(`Please share the report in ${chalk.bold('#experimental-bundler-beta')} slack channel, this will help us to roll it out to production sooner!`);
  } catch (error) {
