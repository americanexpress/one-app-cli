...
describe('getConfigOptions', () => {
  ...
  it('should warn when the user provides a custom webpack config', () => {
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith('@americanexpress/one-app-server-bundler: Using a custom webpack config can cause unintended side effects. Issues resulting from custom configuration will not be supported.');
  });
  ...
  it('should throw when a user attempts to use both webpackConfigPath and webpackClientConfigPath', () => {
    const errorRegex = /@americanexpress\/one-app-server-bundler: Modules cannot configure both webpackConfigPath and webpackClientConfigPath or webpackServerConfigPath. See README for details./;
