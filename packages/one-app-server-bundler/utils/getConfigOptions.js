...
function validateOptions(options) {
  if (options.requiredExternals && options.providedExternals) {
    throw new Error('@americanexpress/one-app-server-bundler: Modules cannot configure both requiredExternals and providedExternals. See README for details.');
  }
  ...
  if (options.requiredExternals || options.providedExternals) {
    ...
    if (intersection.length > 0) {
      throw new Error(`@americanexpress/one-app-server-bundler: Attempted to bundle ${intersection.join(', ')}, but modules cannot provide externals that One App includes.`);
    }
  }
  ...
  if (
    ...
    throw new Error('`@americanexpress/one-app-server-bundler: Modules cannot configure both webpackConfigPath and webpackClientConfigPath or webpackServerConfigPath. See README for details.');
  }
}
...
function logConfigurationWarnings(options) {
  if (options.webpackConfigPath
    ...
    console.warn('@americanexpress/one-app-server-bundler: Using a custom webpack config can cause unintended side effects. Issues resulting from custom configuration will not be supported.');
  }
}
